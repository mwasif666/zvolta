document.addEventListener("DOMContentLoaded", function () {
  ...
  if (wordContainer) {
    ...
    setInterval(() => {
      ...
      tl.to(
        wordContainer,
        { width: newWidth, duration: 0.6, ease: "expo.inOut" },
        0,
      );
      if (currentSpan) {
        tl.to(
  ...
  if (wrapper && content) {
    ...
    scrollObserver = Observer.create({
      ...
      onChange: (self) => {
        const activeLoader = document.getElementById("custom-loader");
        if (activeLoader && activeLoader.style.display !== "none") return;
  ...
  if (howSection) {
    ...
    cardColumns.forEach((col) => {
      ...
      if (state1 && state2)
        gsap.set([state1, state2], { autoAlpha: 0, y: 30, zIndex: 0 });
    });

    if (btnGroups.length >= 3) {
      gsap.set([btnGroups[1], btnGroups[2]], {
        y: 20,
        autoAlpha: 0,
        pointerEvents: "none",
      });
        ...
        spans.forEach((span, index) => {
          ...
          if (progress >= 1 || progress > wordPos + spread) {
            ...
            const transitionState =
              (progress - (wordPos - spread)) / (spread * 2);
  ...
  if (teamSection) {
    ...
    tl.to(contents[0], {
      ...
      contents[1],
      { height: heights[1], opacity: 1, duration: 1, ease: "power2.inOut" },
      "<",
    );
    ...
    tl.to(contents[1], {
      ...
      contents[2],
      { height: heights[2], opacity: 1, duration: 1, ease: "power2.inOut" },
      "<",
    );
  ...
  if (contactSection) {
    ...
    const contactSectionRoot = contactSection.querySelector(
